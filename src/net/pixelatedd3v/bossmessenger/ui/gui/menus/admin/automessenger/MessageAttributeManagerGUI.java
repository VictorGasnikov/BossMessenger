...
public class MessageAttributeManagerGUI extends ChestGUI {
	...
	private Runnable cancel = new Runnable() {
		...
		public void run() {
			UISession session = getSession();
			session.openUI(new MessageAttributeManagerGUI(session, messenger, messageId));
		}
	};
...
//		COMPONENTS.add(new GUIComponent(ItemUtils.createStack(Material.BARRIER, 1, "§cDelete"), 51));
	...
	public MessageAttributeManagerGUI(UISession session, Messenger messenger, int messageId) {
...
//		inv.add(new GUIComponent(ItemUtils.createStack(Material.NAME_TAG, 1, "§eAttributes", "" + Lang.CLICK_TO_EDIT), 33));
