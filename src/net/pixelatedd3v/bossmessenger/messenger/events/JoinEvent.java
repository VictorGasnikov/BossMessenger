...
public class JoinEvent extends Event {
	...
	public JoinEvent(String name, boolean enabled, String text, int show, List<String> messengerNames, Collection<MessageAttribute> attributes) {
		super(name, enabled, text, show, messengerNames, attributes);
	}

	public void fire(Player player) {
		if (isEnabled() && VnpManager.getInstance().isVisible(player)) {
			TimedMessage message = new TimedMessage(getText().replaceAll("%target%", player.getName()), getShow(), getAttributes().values());
